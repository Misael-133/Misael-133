# <h1 align="center">Olá! Eu sou o Misael Gonçalves 👋</h1>

<p align="center">
  Estudante de Análise e Desenvolvimento de Sistemas • Desenvolvedor Full Stack em formação • Python • Flask • SQL • APIs REST • Power BI
</p>

<p align="center">
  <a href="https://github.com/Misael-133">
    <img src="https://komarev.com/ghpvc/?username=Misael-133&label=Visualiza%C3%A7%C3%B5es&color=0e75b6&style=flat" alt="contador de visualizações" />
  </a>
  <a href="https://www.linkedin.com/in/misael-gon%C3%A7alves/">
    <img src="https://img.shields.io/badge/LinkedIn-Conectar-blue?style=flat&logo=linkedin" alt="LinkedIn" />
  </a>
</p>

---

## 🚀 Sobre mim

Sou estudante de **Análise e Desenvolvimento de Sistemas** e estou em transição de carreira para a área de **Tecnologia**.

Minha base profissional vem da indústria, com experiência em **processos, melhoria contínua, análise, resolução de problemas e rotina operacional**, e hoje direciono essa bagagem para o desenvolvimento de soluções com foco em:

* Desenvolvimento Full Stack
* Python
* Flask e APIs REST
* SQL e Banco de Dados
* Power BI
* Lógica de programação
* Automação e análise de dados

Gosto de criar projetos que simulam cenários reais de empresas, reforçando minha evolução prática como futuro desenvolvedor.

---

## 🛠️ Tecnologias e ferramentas

<p>
  <img src="https://skillicons.dev/icons?i=python,flask,mysql,sqlite,git,github,vscode" />
</p>

<p>
  <img src="https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black" />
  <img src="https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white" />
</p>

---

## 📌 Projetos em destaque

### 🔹 [Desafio-Python](https://github.com/Misael-133/Desafio-Python)

Repositório com exercícios práticos em Python, cobrindo lógica de programação, estruturas condicionais, loops, cálculos e simulações de problemas reais.

**Destaques:**

* Entrada e saída de dados
* `if/else`, `for`, `while`
* Cálculo de corrida e métricas como **R$/km**
* Sistema com menu interativo

---

### 🔹 [Sistema de Ordem de Serviço (Banco de Dados SQL)](https://github.com/Misael-133/Sistema-de-Ordem-de-Servi-o-Banco-de-Dados-SQL-)

Sistema de banco de dados relacional para gerenciamento de ordens de serviço, desenvolvido em SQL, com modelagem completa, relacionamentos entre tabelas e consultas utilizando JOIN, GROUP BY e SUM.

---

### 🔹 [python-corridas-sqlite](https://github.com/Misael-133/python-corridas-sqlite)

Projeto em Python com integração a banco de dados SQLite para registro e análise de corridas, aplicando persistência de dados, consultas SQL e operações de agregação (SUM e AVG), com foco em desenvolvimento backend e estruturação de dados.

---

### 🔹 [api-gerenciamento-chamados](COLE_SEU_LINK_AQUI)

API REST desenvolvida com Flask e SQLite simulando um sistema corporativo de gerenciamento de chamados técnicos.

**Funcionalidades:**

* Criação de chamados via API
* Listagem de chamados cadastrados
* Integração entre Flask e SQLite
* Retorno de dados em JSON
* Estrutura backend com rotas GET e POST

---

### 🔹 [Gerenciador_Contatos_python](https://github.com/Misael-133/Gerenciador_Contatos_python)

Projeto em Python voltado para organização e manipulação de contatos, reforçando lógica, estruturação de dados e fluxo de navegação.

---

### 🔹 [Sistema-de-Vendas-de-Madeira](https://github.com/Misael-133/Sistema-de-Vendas-de-Madeira)

Projeto prático com foco em regras de negócio, cálculos e estruturação de processo de venda.

---

### 🔹 [calculadora-plano-python](https://github.com/Misael-133/calculadora-plano-python)

Projeto em Python voltado para cálculos e apoio à tomada de decisão, reforçando lógica aplicada.

---

## 📈 Em evolução

Atualmente estou aprofundando meus estudos em:

* Desenvolvimento Backend com Python e Flask
* APIs REST
* Banco de dados relacionais (SQL e SQLite)
* Desenvolvimento Front-end
* Git e GitHub
* Power BI e análise de dados
* Desenvolvimento de projetos práticos para portfólio

---

## 🎯 Objetivo

Busco uma oportunidade de estágio ou posição júnior em Desenvolvimento Full Stack, Backend ou Dados, onde eu possa aplicar minha capacidade analítica, disciplina e aprendizado rápido na construção de soluções reais com tecnologia.

---

## 📫 Contato

* **GitHub:** https://github.com/Misael-133
* **LinkedIn:** https://www.linkedin.com/in/misael-gon%C3%A7alves/
* **E-mail:** [misaelgama@gmail.com](mailto:misaelgama@gmail.com)

---

## ⚡ Frase que representa meu momento

> Transformando experiência prática, disciplina e vontade de evoluir em soluções reais com tecnologia.
